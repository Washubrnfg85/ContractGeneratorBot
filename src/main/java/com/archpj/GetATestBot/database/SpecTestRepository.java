...
public interface SpecTestRepository extends JpaRepository<SpecTest, Integer> {
    ...
    @Query(value = "select question from spec_test where spec = ?1", nativeQuery = true)
    List<String> loadSpecQuestions(String spec);

    @Query(value = "select correct_answer from spec_test where spec = ?1", nativeQuery = true)
