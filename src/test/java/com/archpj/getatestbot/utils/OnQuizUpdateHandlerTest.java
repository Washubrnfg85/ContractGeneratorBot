...
public class OnQuizUpdateHandlerTest {
    ...
    void shouldReturnErrorTextIfMessageReceived() {
        ...
        Assertions.assertEquals(OnQuizUpdateHandler.handleUpdate(update).getText(), BotCommands.ON_QUIZ_ERROR_TEXT);
    }

    @Test
    void shouldReturnErrorTextIfWrongCallbackReceived() {
        Mockito.when(update.hasMessage()).thenReturn(false);
        ...
        Mockito.when(update.getCallbackQuery().getData()).thenReturn("");
